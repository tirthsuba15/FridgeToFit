...
// Accepts: multipart image OR JSON body { ingredients: [...] } OR body.text (comma-separated)
router.post('/extract', upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // JSON array path
      ingredientNames = req.body.ingredients.filter(n => typeof n === 'string' && n.trim());
      ...
      return res.status(400).json({ error: 'image, ingredients array, or text required' });
    }
    ...
    for (const name of ingredientNames) {
      ...
      const row = selectStmt.get(clean);
      if (row) ingredients.push({ id: row.id, name: row.name, quantity_g: null });
    }

    return res.json({ ingredients, saved: ingredients });
  } catch (e) {
